...
const CGFloat POLYLINE_WIDTH = 7.0;
...
-(void) regionChanged {
	...
	for (int i = 0; i < annotation.points.count; i++)
	{
		...
		if (i == 0 || point.x < minpt.x)
			minpt.x = point.x;
		if (i == 0 || point.y < minpt.y)
			minpt.y = point.y;
		if (i == 0 || point.x > maxpt.x)
			maxpt.x = point.x;
		if (i == 0 || point.y > maxpt.y)
			maxpt.y = point.y;
	}
	...
	_internalView.frame = CGRectMake(minpt.x - POLYLINE_WIDTH, minpt.y - POLYLINE_WIDTH, w, h);
